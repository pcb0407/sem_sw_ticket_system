...
try { $host.UI.RawUI.WindowTitle = 'sem sw ticket system Backend Debug' } catch {}
...
function Write-ApphostConsoleState([string]$status, [int]$exitCode = 0, [string]$message = '') {
  $json = @{ status = $status; exitCode = $exitCode; message = $message; command = 'start-apphost-service.ps1 -Target backend-debug'; pid = $PID; updatedAt = (Get-Date).ToString('o') } | ConvertTo-Json -Compress
  Write-ApphostConsoleStateFile 'D:\sem_sw_portal\sem_sw_ticket_system\.vscode\.logs\backend-dev.state.json' $json
}
Write-ApphostConsoleState 'starting' 0 ''
try { Start-Transcript -Path 'D:\sem_sw_portal\sem_sw_ticket_system\.vscode\.logs\backend-dev.log' -Force | Out-Null } catch { Write-Warning $_.Exception.Message }
$apphostCleanupTarget = ''
$apphostWorkspace = 'D:\sem_sw_portal\sem_sw_ticket_system'
try {
  Set-Location -LiteralPath 'D:\sem_sw_portal\sem_sw_ticket_system'
  & 'D:\sem_sw_portal\sem_sw_ticket_system/.vscode/scripts/start-apphost-service.ps1' -Target backend-debug
