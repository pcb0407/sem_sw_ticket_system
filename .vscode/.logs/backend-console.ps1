...
try { $host.UI.RawUI.WindowTitle = 'sem sw ticket system Backend Dev' } catch {}
...
function Write-ApphostConsoleState([string]$status, [int]$exitCode = 0, [string]$message = '') {
  $json = @{ status = $status; exitCode = $exitCode; message = $message; command = 'start-apphost-service.ps1 -Target backend'; pid = $PID; updatedAt = (Get-Date).ToString('o') } | ConvertTo-Json -Compress
...
try {
  ...
  & 'D:\sem_sw_portal\sem_sw_ticket_system/.vscode/scripts/start-apphost-service.ps1' -Target backend
